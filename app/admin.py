...
class DefaultUserAdmin(ModelAdminMixin, UserAdmin):
    ...
    def add_view(self, request, form_url='', extra_context=None):
        return super(DefaultUserAdmin, self).add_view(request, form_url, extra_context)

    def change_view(self, request, object_id, form_url='', extra_context=None):
        return super(DefaultUserAdmin, self).change_view(request, object_id, form_url, extra_context)

    def delete_view(self, request, object_id, extra_context=None):
        return super(DefaultUserAdmin, self).delete_view(request, object_id, extra_context)

    def changelist_view(self, request, extra_context=None):
        return super(DefaultUserAdmin, self).changelist_view(request=request, extra_context=extra_context)
...
class QuizAdmin(ModelAdminMixin, admin.ModelAdmin):
    ...
    def add_view(self, request, form_url='', extra_context=None):
        return super(QuizAdmin, self).add_view(request, form_url, extra_context)

    def change_view(self, request, object_id, form_url='', extra_context=None):
        return super(QuizAdmin, self).change_view(request, object_id, form_url, extra_context)

    def delete_view(self, request, object_id, extra_context=None):
        return super(QuizAdmin, self).delete_view(request, object_id, extra_context)

    def changelist_view(self, request, extra_context=None):
        return super(QuizAdmin, self).changelist_view(request=request, extra_context=extra_context)
...
class QuizResultsAdmin(ModelAdminMixin, admin.ModelAdmin):
    ...
    def add_view(self, request, form_url='', extra_context=None):
        return super(QuizResultsAdmin, self).add_view(request, form_url, extra_context)

    def change_view(self, request, object_id, form_url='', extra_context=None):
        return super(QuizResultsAdmin, self).change_view(request, object_id, form_url, extra_context)

    def delete_view(self, request, object_id, extra_context=None):
        return super(QuizResultsAdmin, self).delete_view(request, object_id, extra_context)

    def changelist_view(self, request, extra_context=None):
        return super(QuizResultsAdmin, self).changelist_view(request=request, extra_context=extra_context)
...
class LectureAdmin(ModelAdminMixin, admin.ModelAdmin):
    ...
    def add_view(self, request, form_url='', extra_context=None):
        return super(LectureAdmin, self).add_view(request, form_url, extra_context)

    def change_view(self, request, object_id, form_url='', extra_context=None):
        return super(LectureAdmin, self).change_view(request, object_id, form_url, extra_context)

    def delete_view(self, request, object_id, extra_context=None):
        return super(LectureAdmin, self).delete_view(request, object_id, extra_context)

    def changelist_view(self, request, extra_context=None):
        return super(LectureAdmin, self).changelist_view(request=request, extra_context=extra_context)
...
class UserProfileAdmin(ModelAdminMixin, admin.ModelAdmin):
    ...
    def delete_view(self, request, object_id, extra_context=None):
        return super(UserProfileAdmin, self).delete_view(request, object_id, extra_context)
...
class WordcloudAdmin(ModelAdminMixin, admin.ModelAdmin):
    ...
    def delete_view(self, request, object_id, extra_context=None):
        return super(WordcloudAdmin, self).delete_view(request, object_id, extra_context)

    def changelist_view(self, request, extra_context=None):
        return super(WordcloudAdmin, self).changelist_view(request=request, extra_context=extra_context)
...
class CodeSnippetAdmin(ModelAdminMixin, admin.ModelAdmin):
    ...
    def add_view(self, request, form_url='', extra_context=None):
        return super(CodeSnippetAdmin, self).add_view(request, form_url, extra_context)

    def change_view(self, request, object_id, form_url='', extra_context=None):
        return super(CodeSnippetAdmin, self).change_view(request, object_id, form_url, extra_context)

    def delete_view(self, request, object_id, extra_context=None):
        return super(CodeSnippetAdmin, self).delete_view(request, object_id, extra_context)

    def changelist_view(self, request, extra_context=None):
        return super(CodeSnippetAdmin, self).changelist_view(request=request, extra_context=extra_context)


class DefaultAdminSite(AdminSite):

    def each_context(self, context=None):
        print dir(self)
        return super(DefaultAdminSite, self).each_context()
